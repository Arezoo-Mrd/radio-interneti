import { z } from "zod";

export const createLiveSchema = z.object({
    name: z.string().min(1, "وارد کردن موضوع لایو الزامی است"),
    description: z.string().optional(),
    start_date: z.instanceof(Date),
    end_date: z.instanceof(Date),
    start_time: z.string().min(1, "زمان شروع الزامی است").refine((time) => {
        const [hours, minutes] = time.split(':').map(Number);
        return hours >= 0 && hours < 24 && minutes >= 0 && minutes < 60;
    }, { message: "زمان شروع معتبر نیست" }),
    end_time: z.string().min(1, "زمان پایان الزامی است").refine((time) => {
        const [hours, minutes] = time.split(':').map(Number);
        return hours >= 0 && hours < 24 && minutes >= 0 && minutes < 60;
    }, { message: "زمان پایان معتبر نیست" }),
    activate: z.boolean().optional(),
    presenter: z.array(z.object({
        name: z.string(),
    })).optional(),

});

export type CreateLiveSchemaType = z.infer<typeof createLiveSchema>;
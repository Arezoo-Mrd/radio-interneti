...
const Dashboard = ({ dashboardInfo, logs }: { dashboardInfo: DashboardInfo | undefined, logs: string[] }) => {
    ...
    const sortedVisitors = useMemo(() => dashboardInfo?.visitors?.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()), [dashboardInfo]);
